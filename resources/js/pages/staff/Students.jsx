/**
 * Petugas - Data Siswa (Realtime)
 * Uses shared StudentContext — automatically synced with Admin portal.
 * Petugas can add/edit students but cannot delete.
 */
import { useState, useRef } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import {
  Users, Plus, Search, Edit2, Loader2, X, Camera,
  CheckCircle2, Lock, XCircle, RefreshCw, Wifi, ChevronLeft, ChevronRight
} from 'lucide-react';
import api from '../../api';
...
export default function PetugasStudents() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingStudent) {
        ...
        toast.success('✅ Data siswa diperbarui — sinkron ke admin!');
      } else {
        await api.post('/students', fd, { headers: { 'Content-Type': 'multipart/form-data' } });
        toast.success('✅ Siswa baru didaftarkan!');
      }
      setModalOpen(false);
      triggerPulse();
      refreshStudents();
  ...
  const inp = "w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-emerald-500 transition-all";
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-black text-slate-900 flex items-center gap-2">
            Data Siswa / Anggota
            <span className={`inline-flex items-center gap-1.5 text-xs font-bold px-2.5 py-1 rounded-full transition-all duration-300 ${pulse ? 'bg-emerald-200 text-emerald-900 scale-110' : 'bg-emerald-100 text-emerald-700'}`}>
              <Wifi size={10} /> LIVE SYNC
            </span>
          </h1>
          <p className="text-sm text-slate-500 mt-1">Realtime sinkron dengan dashboard admin · Polling setiap 8 detik</p>
        </div>
      ...
      <div className="flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
          <div className="relative w-full sm:w-72">
            ...
            <input defaultValue={params.search} onChange={e => handleSearch(e.target.value)}
              placeholder="Cari nama, NISN, email..."
              className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-emerald-500 transition-all" />
          </div>
          <select value={params.jurusan} onChange={e => setParams(p => ({ ...p, jurusan: e.target.value, page: 1 }))}
            className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-emerald-500">
        ...
        <button onClick={() => openForm()} className="flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-xl font-bold text-sm shadow-lg shadow-emerald-200 transition-colors">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Total Siswa', value: meta.total || 0 },
          { label: 'Halaman', value: `${meta.current_page || 1} / ${meta.last_page || 1}` },
          { label: 'Status', value: loading ? 'Memuat...' : 'Live ✓' },
        ].map(s => (
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-24">
            ...
            <p className="text-slate-500 font-bold">Belum ada data siswa</p>
          </div>
        ) : (
                  ...
                  <motion.tr key={s.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <img src={s.profile_photo_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(s.name)}&background=059669&color=fff`}
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-center">
                        <button onClick={() => openForm(s)} className="w-8 h-8 rounded-lg bg-slate-100 text-slate-600 hover:bg-emerald-100 hover:text-emerald-600 flex items-center justify-center transition-colors" title="Edit">
                          <Edit2 size={14} />
                        </button>
                      </div>
                    </td>
        ...
        {meta.last_page > 1 && (
          <div className="border-t border-slate-100 px-6 py-4 flex items-center justify-between">
            <span className="text-sm text-slate-500 font-medium">Halaman {meta.current_page} dari {meta.last_page} · {meta.total} siswa</span>
            <div className="flex items-center gap-1">
              ...
              {Array.from({ length: Math.min(meta.last_page, 7) }, (_, i) => i + 1).map(p => (
                <button key={p} onClick={() => setParams(pr => ({ ...pr, page: p }))}
                  className={`w-8 h-8 rounded-lg text-sm font-bold transition-colors ${params.page === p ? 'bg-emerald-600 text-white' : 'text-slate-600 hover:bg-slate-100'}`}>
                  {p}
                </button>
              ))}
        ...
        {modalOpen && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 overflow-y-auto">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative bg-white rounded-3xl shadow-2xl max-w-2xl w-full my-8">
              <div className="p-6 border-b border-slate-100 flex justify-between items-center">
                <div>
                  <h3 className="text-xl font-black text-slate-900">{editingStudent ? 'Edit Data Siswa' : 'Registrasi Siswa Baru'}</h3>
                  <p className="text-xs text-slate-400 mt-0.5">Perubahan langsung sinkron ke admin</p>
                </div>
                <button onClick={() => setModalOpen(false)} className="p-2 bg-slate-100 text-slate-500 hover:bg-slate-200 rounded-xl"><X size={18} /></button>
              </div>
              <form onSubmit={handleSubmit} className="p-6 space-y-5">
                <div className="flex gap-6 items-start">
                  <div className="shrink-0 flex flex-col items-center">
                    <label className="relative cursor-pointer group">
                      <div className="w-24 h-24 rounded-full overflow-hidden border-2 border-dashed border-slate-300 group-hover:border-emerald-500 bg-slate-50 flex items-center justify-center transition-colors">
                        {photoPreview ? <img src={photoPreview} className="w-full h-full object-cover" alt="Preview" /> : <Camera size={24} className="text-slate-400 group-hover:text-emerald-500" />}
                      </div>
                ...
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="flex flex-col justify-end">
                    <label className="flex items-center gap-2 cursor-pointer h-10">
                      <input type="checkbox" checked={form.is_active} onChange={e => setForm(f => ({ ...f, is_active: e.target.checked }))} className="w-5 h-5 rounded border-slate-300 text-emerald-600" />
                ...
                <div className="pt-2 flex justify-end gap-3">
                  ...
                  <button type="submit" disabled={formLoading} className="px-6 py-2.5 bg-emerald-600 text-white font-bold rounded-xl text-sm shadow-lg shadow-emerald-200 flex items-center gap-2 disabled:opacity-60 hover:bg-emerald-700 transition-colors">
