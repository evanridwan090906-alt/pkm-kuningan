...
export default function LoginPetugas() {
  ...
  const handleLogin = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/login', { identifier, password, role: 'petugas' });
      const { access_token, user } = res.data.data;

      if (user.role !== 'petugas' && user.role !== 'admin') {
        toast.error('Akses ditolak. Anda bukan Petugas.');
        setLoading(false);
        return;
      }
      ...
      toast.success('Shift Dimulai!', { icon: '✅' });
      setTransition('petugas'); // trigger right-slide

      setTimeout(() => navigate('/petugas/dashboard'), 900);
    } catch (err) {
      toast.error(err.response?.data?.message || 'Login gagal, periksa ID Anda');
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-white flex overflow-hidden font-sans">
      ...
      {/* Left hero — exits right on login (petugas: right direction) */}
      <AnimatePresence>
        {!loading && (
          <motion.div
            ...
            className="hidden lg:flex w-1/2 relative bg-[#064e3b] items-center justify-center p-12 overflow-hidden"
          >
            ...
            <div className="absolute inset-0 z-[5] bg-gradient-to-br from-emerald-900/30 via-transparent to-teal-900/20" />
            ...
            <div className="relative z-20 max-w-md text-center lg:text-left drop-shadow-lg">
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
                ...
                {system.school_name && <p className="text-xl text-emerald-300 font-bold mb-4">{system.school_name}</p>}
      ...
      {/* Right form — enters from left when dashboard loads */}
      <motion.div
      ...
      >
          ...
          {loading && (
            ...
            >
              <div className="flex flex-col items-center gap-3">
                ...
                <p className="text-sm font-bold text-slate-700">Memulai Shift...</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="w-full max-w-md">
          <div className="mb-10 text-center lg:text-left">
            ...
            <h2 className="text-4xl font-black text-slate-900 tracking-tight mb-3">Selamat Bertugas</h2>
            <p className="text-slate-500 font-medium">Silakan masuk untuk memulai shift di {system.app_name}</p>
          </div>

          <form onSubmit={handleLogin} className="space-y-6">
            <div className="space-y-5">
              <div>
                <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">ID Petugas / Email</label>
                <div className="relative group">
                  ...
                  <input type="text" required value={identifier} onChange={(e) => setIdentifier(e.target.value)}
                    className="w-full pl-12 pr-4 py-4 bg-white lg:bg-slate-50 border border-slate-200 rounded-2xl focus:outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-600 transition-all font-bold text-sm"
            ...
            <motion.button
              type="submit"
              disabled={loading}
              whileHover={!loading ? { scale: 1.01, boxShadow: '0 8px 30px rgba(5,150,105,0.2)' } : {}}
              whileTap={!loading ? { scale: 0.98 } : {}}
              className="w-full py-4 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-black text-sm transition-colors flex items-center justify-center gap-2 mt-4 shadow-xl shadow-emerald-900/10 disabled:opacity-60"
            >
              {loading ? (
